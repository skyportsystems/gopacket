...
func NewDHCP(xid uint32, op DHCPOperation, hwtype byte) (*DHCPv4, error) {
	if xid == 0 {
		xid = rand.Uint32()
	}
	switch hwtype {
	case DHCP_HW_TYPE_ETHERNET:
		break
	default:
...
func NewDHCPDiscover(xid uint32) (*DHCPv4, error) {
	p, err := NewDHCP(xid, DHCP_MSG_REQ, DHCP_HW_TYPE_ETHERNET)
	if err != nil {
		return nil, err
	}
	p.Options = append(p.Options, DHCPOption{Type: 53, Data: []byte{byte(DHCP_MSG_DISCOVER)}})
	return p, nil
}

func NewDHCPOffer(xid uint32) (*DHCPv4, error) {
	p, err := NewDHCP(xid, DHCP_MSG_RES, DHCP_HW_TYPE_ETHERNET)
	if err != nil {
		return nil, err
	}
	p.Options = append(p.Options, NewDHCPOption(53, []byte{byte(DHCP_MSG_OFFER)}))
	return p, nil
}

func NewDHCPRequest(xid uint32) (*DHCPv4, error) {
	p, err := NewDHCP(xid, DHCP_MSG_REQ, DHCP_HW_TYPE_ETHERNET)
	if err != nil {
		return nil, err
	}
	p.Options = append(p.Options, DHCPOption{Type: 53, Data: []byte{byte(DHCP_MSG_REQUEST)}})
	return p, nil
}

func NewDHCPAck(xid uint32) (*DHCPv4, error) {
	p, err := NewDHCP(xid, DHCP_MSG_RES, DHCP_HW_TYPE_ETHERNET)
	if err != nil {
		return nil, err
	}
	p.Options = append(p.Options, NewDHCPOption(53, []byte{byte(DHCP_MSG_ACK)}))
	return p, nil
}

func NewDHCPNak(xid uint32) (*DHCPv4, error) {
	p, err := NewDHCP(xid, DHCP_MSG_RES, DHCP_HW_TYPE_ETHERNET)
	if err != nil {
		return nil, err
	}
	p.Options = append(p.Options, DHCPOption{Type: 53, Data: []byte{byte(DHCP_MSG_NAK)}})
	return p, nil
}
